...
function addUsersHandler(req, res) {
  ...
  client
    .query(checkEmailSql, checkEmailValues)
    .then((result) => {
...
function getAllPostsHandler(req, res) {
  const sql =
    "SELECT Posts.postId ,Users.userId ,Users.userFullName, Users.imageURL AS userImageURL , Posts.postId  , Posts.imageURL , Posts.title , Posts.content  , Posts.numberOfLikes , Posts.Created_at  FROM Users INNER JOIN Posts ON Users.userId=Posts.userId  ORDER BY Created_at DESC ;";
  client
    .query(sql)
    .then((data) => {
      res.send(data.rows);
...
function deletePostHandler(req, res) {
  ...
  if (!isNaN(postId)) {
    ...
    client
      .query(deleteCommentsQuery, [postId])
      .then(() => client.query(deletePostQuery, [postId]))
      .then(() =>
        res.send("Post and associated comments deleted successfully.")
      )
